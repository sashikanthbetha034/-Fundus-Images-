...
# P = (sum(initsol)/Population) + ((current fit + best fit)/(worst fit + mean fit))
# updated in line 27
def MLEA(initsol, fname, xmin, xmax, Max_iter):
    ...
    for t in range(Max_iter):
        for i in range(Npop):
            P = (sum(X) / Npop) + ((fitness[i] + np.min(fitness)) / (np.max(fitness) + np.mean(fitness)))
            if P <= 0.5:
                ...
                if np.linalg.norm(X[i] - g_best) <= 1.0:
                    ...
                    X[i] = X[i] + np.random.uniform(-0.1, 0.1, size=X[i].shape)
